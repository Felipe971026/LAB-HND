...
export const RecepcionForm: React.FC<RecepcionFormProps> = ({ onSubmit, isSubmitting }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (let i = 0; i < units.length; i++) {
      const u = units[i];
      if (!u.hemoderivativeType || !u.unitId || !u.bloodGroup || !u.rh || !u.expirationDate || !u.accepted) {
        setError(`Por favor complete los campos obligatorios de la unidad ${i + 1} (Tipo, Número de bolsa, Grupo, Rh, Vencimiento, Aceptación).`);
        return;
      }
                      ...
                      {unit.accepted === 'No' && (
                        <div className="mt-4 p-4 bg-red-50 rounded-xl border border-red-100 space-y-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="md:col-span-2">
                              <label className="block text-sm font-medium text-red-800 mb-2">Acciones tomadas</label>
                              <input
                                type="text"
                                name="actionsTaken"
                            ...
                            <div className="md:col-span-2">
                              <label className="block text-sm font-medium text-red-800 mb-2">Firma / Nombre del encargado del reporte</label>
                              <input
                                type="text"
                                name="reporterName"
                                value={unit.reporterName}
                                onChange={(e) => handleUnitChange(index, e)}
                                className="w-full px-4 py-3 rounded-xl border border-red-200 focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition-all bg-white"
                                required
                              />
