  ...
  React.useEffect(() => {
    const validateCrossMatch = async () => {
      if (formData.patientId && (formData.unitId || formData.qualitySeal)) {
        ...
        if (snapTest.empty) {
          setAlertMessage(`ALERTA DE NOVEDAD: No se encontró una PRUEBA DE COMPATIBILIDAD registrada para el paciente ${formData.patientId} con la unidad ${value}.`);
        }
      }
    };
  ...
  const lookupUnit = async (value: string, type: 'qualitySeal' | 'unitId') => {
    ...
    try {
      ...
      if (formData.patientId) {
        ...
        if (snapTest.empty) {
          setAlertMessage(`ALERTA DE NOVEDAD: La unidad existe en recepción pero NO se encontró una PRUEBA DE COMPATIBILIDAD registrada para el paciente ${formData.patientId} con esta unidad.`);
        }
      }
